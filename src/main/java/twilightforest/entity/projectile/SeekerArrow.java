...
public class SeekerArrow extends TFArrow {
	...
	public void tick() {
		if (isThisArrowFlying()) {
			...
			if (target != null) {
				...
				// vector lengths
				double courseLen = getDeltaMovement().length();
				double targetLen = targetVec.length();
				double totalLen = Math.sqrt(courseLen*courseLen + targetLen*targetLen);

				double dotProduct = getDeltaMovement().dot(targetVec) / (courseLen * targetLen); // cosine similarity

				if (dotProduct > seekThreshold) {

					// add vector to target, scale to match current velocity
					Vec3 newMotion = getDeltaMovement().scale(courseLen / totalLen).add(targetVec.scale(courseLen / totalLen));
	...
	private void updateTarget() {
		...
		if (target == null) {
			...
			Vec3 courseVec = getDeltaMovement().scale(seekDistance).yRot((float) seekAngle);
			targetBB = targetBB.minmax(positionBB.move(courseVec));

			courseVec = getDeltaMovement().scale(seekDistance).yRot((float) -seekAngle);
			targetBB = targetBB.minmax(positionBB.move(courseVec));
