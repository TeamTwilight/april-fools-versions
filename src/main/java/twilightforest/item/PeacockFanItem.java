...
public class PeacockFanItem extends Item {
	...
	public InteractionResultHolder<ItemStack> use(Level world, Player player, @Nonnull InteractionHand hand) {
		...
		if (!world.isClientSide) {
			...
			if (!player.isOnGround() && !player.isSwimming() && !launched) {
				...
				launched = true;
				elytraOnLaunch = player.getInventory().armor.get(2).is(Items.ELYTRA);
			} else {
	...
	public void inventoryTick(ItemStack stack, Level worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
		if(elytraOnLaunch && entityIn instanceof Player player && !player.getInventory().armor.get(2).is(Items.ELYTRA)) {
			player.fallDistance = Float.MAX_VALUE;
			launched = false;
			elytraOnLaunch = false;
			return;
		}
