...
public class TFWeatherRenderer {
	...
	public static boolean YETI = false;
	public static boolean UR_GHAST = false;
	public static boolean KNIGHTS = false;
	public static boolean HYDRA = false;
	public static boolean MINO = false;
	public static boolean LICH = false;
	public static boolean NAGA = false;


	// [VanillaCopy] inside of EntityRenderer.renderRainSnow, edits noted
	private static void renderLockedBiome(float partialTicks, ClientLevel level, Minecraft mc, LightTexture lightmap, double xIn, double yIn, double zIn) {
		if (mc.getCameraEntity() instanceof LocalPlayer player) {
			RandomSource ra = player.getRandom();
			...
			if (player.tickCount % 100 == 0) {
				QUEEN = PlayerHelper.doesPlayerHaveRequiredAdvancement(player, TwilightForestMod.prefix("progress_glacier"));
				YETI = PlayerHelper.doesPlayerHaveRequiredAdvancement(player, TwilightForestMod.prefix("progress_yeti"));
				UR_GHAST = PlayerHelper.doesPlayerHaveRequiredAdvancement(player, TwilightForestMod.prefix("progress_ur_ghast"));
				KNIGHTS = PlayerHelper.doesPlayerHaveRequiredAdvancement(player, TwilightForestMod.prefix("progress_knights"));
				HYDRA = PlayerHelper.doesPlayerHaveRequiredAdvancement(player, TwilightForestMod.prefix("progress_hydra"));
				MINO = PlayerHelper.doesPlayerHaveRequiredAdvancement(player, TwilightForestMod.prefix("progress_labyrinth"));
				LICH = PlayerHelper.doesPlayerHaveRequiredAdvancement(player, TwilightForestMod.prefix("progress_lich"));
				NAGA = PlayerHelper.doesPlayerHaveRequiredAdvancement(player, TwilightForestMod.prefix("progress_naga"));
				mc.updateTitle();
			}

			if (!mc.isPaused()) {
				int maxI = (QUEEN ? 1 : 0) + (YETI ? 2 : 0);
				for (int i = 0; i < maxI; i++) {
					double x = xIn + (0.5D - ra.nextDouble()) * 32.0D;
					double y = yIn + (0.5D - ra.nextDouble()) * 32.0D;
					double z = zIn + (0.5D - ra.nextDouble()) * 32.0D;
					if (level.canSeeSky(new BlockPos(x, y, z))) {
						level.addParticle(ParticleTypes.SNOWFLAKE, x, y, z, Mth.randomBetween(ra, -0.15F, 0.15F) * 0.083333336F, -0.05D, Mth.randomBetween(ra, -0.15F, 0.15F) * 0.083333336F);
					}
				}
			}
			...
			int range = 6;
			if (Minecraft.useFancyGraphics()) range = 10;
			...
			for (int z = z0 - range; z <= z0 + range; ++z) {
				for (int x = x0 - range; x <= x0 + range; ++x) {
					...
					if (minY != maxY) {
						...
						RenderType nextType = getRenderType(x, z);
	...
	@Nullable
	private static RenderType getRenderType(int x, int z) {//TODO: coordinates seeded?
		if (Minecraft.getInstance().level == null) return null;
		if (KNIGHTS && x % 16 == 0 && z % 16 == 0) return RenderType.DARK_STREAM;
		if (UR_GHAST && x % 32 == 0 && z % 32 == 0) return RenderType.DARK_STREAM;
		if (MINO && (x + 4) % 4 == 0 && (z + 4) % 4 == 0) return RenderType.ASHES;
		if (HYDRA && (x + 4) % 8 == 0 && (z + 4) % 8 == 0)  return RenderType.ASHES;
	...
	private static int dev(int i, int s) {
		return Math.max(1, i / Math.max(1, s));
	}
