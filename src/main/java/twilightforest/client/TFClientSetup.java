...
public class TFClientSetup {
	...
	public static class ForgeEvents {
		...
		@SubscribeEvent
		public static void checkForShittyMod(ClientPlayerNetworkEvent.LoggedInEvent evt) {
			if(optifinePresent || TFConfig.CLIENT_CONFIG.forceOFCompat.get()) {
				evt.getPlayer().getCapability(CapabilityList.OPTITRASH).ifPresent(cap -> cap.setHasOptifine(true));
				evt.getPlayer().sendMessage(new TextComponent("Thank you for installing Optifine."), evt.getPlayer().getUUID());
			} else {
				evt.getPlayer().getCapability(CapabilityList.OPTITRASH).ifPresent(cap -> cap.setHasOptifine(false));
				evt.getPlayer().sendMessage(new TextComponent("Optifine is not installed on this machine. Install it for new compat!"), evt.getPlayer().getUUID());
			}
		}
